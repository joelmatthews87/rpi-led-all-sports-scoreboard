...
| Games                    | ...games.include_preseason                                     | If preseason games should be displayed. Regular season and playoff games are always displayed.                                       | <ul><li>True (Default)</li><li>False</li></ul>                 | Available for NHL, NBA, WNBA, and NFL. Preseason comes from the same endpoints as the rest of the schedule, so enabling it costs no extra API calls. Exhibition opponents without a logo in assets fall back to the league logo. |
| Games                    | ...games.rollover.show_completed_games_until_rollover_end_time | If games for both yesterday and today should be displayed when time is between rollover_start_time_local and rollover_end_time_local. | <ul><li>False (Default)</li><li>True</li>                      | Set True to keep last night's finals on screen through the morning. Games that run past midnight are still shown before rollover_start_time_local regardless of this setting. |
